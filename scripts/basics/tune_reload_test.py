...
Security
Insights
Ray - Tune - Tensorflow / tune_reload_test.py /
...
config[
    "lr"] = 0.0  # the below can't handle tune.grid_search and also a change to config
...
arrays_equal = all(
    [np.array_equal(t, u) for t, u in zip(trained_weights, untrained_weights)])
...
arrays_equal = all(
    [np.array_equal(t, l) for t, l in zip(trained_weights, loaded_weights)])
...
analysis = ExperimentAnalysis(
    "./ppo_test/PPOalgo_2022-10-24_21-23-42")  # put the right name here
...
arrays_equal = all([np.array_equal(t, w) for t, w in zip(trained_weights,
                                                         weights)])  # True => no changes as lr = 0.0

print(f"Trained and recent weights equal? {arrays_equal}")  # True

agent.get_policy().model.base_model.load_weights(
    "my_weights.h5")  # now able to do inferences with rllib
...
class MyModel(tf.keras.Model):
    ...
    def __init__(self, num_actions):
        ...
        self.fc_1 = tf.keras.layers.Dense(num_hidden, activation="relu",
                                          name="fc_1")
        self.fc_value_1 = tf.keras.layers.Dense(num_hidden, activation="relu",
                                                name="fc_value_1")
        
        self.fc_2 = tf.keras.layers.Dense(num_hidden, activation="relu",
                                          name="fc_2")
        self.fc_value_2 = tf.keras.layers.Dense(num_hidden, activation="relu",
                                                name="fc_value_2")
        
        self.fc_out = tf.keras.layers.Dense(num_actions, activation="softmax",
                                            name="fc_out")  # don't forget to include "softmax" here
...
arrays_equal = all(
    [np.array_equal(t, tfw) for t, tfw in zip(trained_weights, tf_weights)])
...
while not done:
    ...
    action = np.random.choice(len(np.squeeze(action_probs)),
